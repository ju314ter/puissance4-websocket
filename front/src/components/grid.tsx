...
const Grid = ({size, playerName, websocket}: GridProps) => {
  ...
  return (
      <div className="grid-container">
          ...
          <div className="grid">
              {gridState.grid.length > 0 && gridState.gridReady && Array(gridSize[1]).fill(0).map((_, indexRow) => {
                  return <div className="grid-row-container" key={"row"+indexRow}>
                      {Array(gridSize[0]).fill(0).map((_,indexCol)=>{
                          return (
