  ...
  startWwebsockets() {
    ...
    wssServer.on("connection", (connection, request) => {
      ...
      connection.on("message", (message) =>
        handleMessage(message, uuid, connectionsCollection)
      );
...
function handleGameMessage(
  message,
  player: Player,
  gameGridCollection: Record<string, Grid>,
  connections
) {
  const gameMessage = <GameMessage>message;
  console.log("New game event !", player.name, gameMessage);

  if (gameMessage.key === "token-drop") {
    // get user grid and drop token
    const grid: Grid = gameGridCollection[player.currentGridId] || null;
  ...
  if (userMessage.gameInfos?.hasStartedGame) {
    ...
    if (hasGridWaiting) {
      ...
      const newGrid = new Grid(
        userMessage.gameInfos.gridSize,
        player,
        gridUuid
      );
      ...
      console.log(
        "Game created, waiting for second player on grid",
        newGrid.gridUuid
      );
    }
  }
